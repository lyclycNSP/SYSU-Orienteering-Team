...
const categories = { '训练回顾': 'training', '比赛故事': 'race', '入门资料': 'guide' };
...
export function articleCard(post, index) {
  ...
  const art = post.cover
    ? `<div class="art"><img src="${escape(assetUrl(post.cover, '../'))}" alt="${escape(post.cover_alt)}" loading="lazy"></div>`
    : `<div class="art ${category}" aria-hidden="true">${String(index + 1).padStart(2, '0')}</div>`;
  return `<article class="card" data-category="${category}"><a href="posts/${encodeURIComponent(post.slug)}.html">${art}<div class="card-body"><span class="tag">${escape(post.category)}${post.example ? ' · 排版示例' : ''}</span><h3>${escape(post.title)}</h3><p>${escape(post.summary)}</p><div class="card-foot"><span>${post.example ? '内容示例' : escape(post.date || post.author || '队伍手记')}</span><span>阅读全文 ↗</span></div></div></a></article>`;
}
...
export function build() {
  ...
  posts.sort((a, b) => String(b.date || '').localeCompare(String(a.date || '')) || b.slug.localeCompare(a.slug));
  const pages = new Map();
  const cards = posts.map(articleCard);
  ...
  for (const post of posts) {
    ...
    pages.set(`team/posts/${post.slug}.html`, template('article.html', { title: escape(post.title), article }));
  }
  const rules = readContent(join(root, 'content/rules/index.md'));
  pages.set('team/rules/index.html', template('article.html', {
    title: escape(rules.title),
    article: `<p class="crumb"><a href="../index.html">队伍手记</a> / 队伍规程</p><header class="article-head"><p class="eyebrow">TEAM HANDBOOK</p><h1>${escape(rules.title)}</h1><p class="small">${escape(rules.summary)}</p></header><div class="prose">${renderMarkdown(rules.body)}</div><a class="back" href="../index.html">← 返回队伍手记</a>`
  ...
  for (const file of ['style.css', 'content.css', 'filter.js', 'map.svg', 'resources.html']) cpSync(join(root, 'team', file), join(output, 'team', file));
  ...
  console.log(`已生成 ${posts.length} 篇文章、队伍首页及规程；指南按原字节复制至 _site。`);
  return output;
}
